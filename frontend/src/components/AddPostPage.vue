          ...
          <form @submit.prevent="handleCreatePost" class="space-y-6">
            <div>
              <div class="flex items-center justify-between mb-2">
                ...
                <span class="text-xs text-gray-500">{{ title.length }}/200</span>
              </div>
              <input
                id="title"
                v-model="title"
                ...
                :class="{ 'border-red-300': title.length > 200 }"
              />
              <p v-if="title.length > 200" class="mt-1 text-xs text-red-600">
            ...
            <div>
              ...
              <textarea
                ...
                :class="{ 'border-red-300': content.length > 5000 }"
              ></textarea>
              <p v-if="content.length > 5000" class="mt-1 text-xs text-red-600">
            ...
            <div class="flex gap-3">
              <button 
                type="submit" 
                :disabled="loading || !title.trim() || !content.trim() || title.length > 200 || content.length > 5000" 
...
const handleCreatePost = async () => {
  ...
  if (title.value.length > 200) {
    error.value = 'Title must not exceed 200 characters'
    return
  }

  if (content.value.length > 5000) {
    error.value = 'Content must not exceed 5000 characters'
    return
  }
